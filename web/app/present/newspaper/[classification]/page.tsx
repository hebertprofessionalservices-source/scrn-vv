...
import { CLASSIFICATIONS, classificationLabel } from "@/lib/team-format";
...
export default async function Newspaper({
  ...
  // Every classification airing in the same episode picks from the same
  // week's results, so their headlines are assigned together and guaranteed
  // not to repeat one another — a sibling class re-running buildNewspaper
  // here is cheap; it's the same in-memory data and ranks already loaded above.
  const leagueClasses = CLASSIFICATIONS.filter((c) => leagueOf(c) === league);
  const headline = weeklyHeadlines(leagueClasses, (c) =>
    (c === classification ? paper : buildNewspaper(data, ranks, { classification: c, dates })).headliners[0] ?? null,
